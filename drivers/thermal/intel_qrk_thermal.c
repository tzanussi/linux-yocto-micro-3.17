...
/* IOSF_MBI ID for thermal */
#define THRM_ID		QRK_MBI_UNIT_RMU
#define GLOBAL_THRM_ID		QRK_MBI_UNIT_SOC

/* IOSF_MBI Opcode for Host Bridge/Remote Monitoring Unit */
#define THRM_CTRL_READ		QRK_MBI_RMU_READ
#define THRM_CTRL_WRITE		QRK_MBI_RMU_WRITE

/* IOSF_MBI Opcode for Host Bridge/Soc Unit */
#define GLB_THRM_CTRL_READ		QRK_MBI_SOC_READ
#define GLB_THRM_CTRL_WRITE		QRK_MBI_SOC_WRITE
...
/* To reset all thermal sensors registers (global reset) */
...
static int intel_qrk_thermal_get_temp(struct thermal_zone_device *tz,
					unsigned long *temp)
{
	iosf_mbi_read(THRM_ID, THRM_CTRL_READ, THRM_TEMP_REG, (u32 *)temp);
...
static int intel_qrk_thermal_probe(struct platform_device *pdev)
{
	...
	iosf_mbi_read(THRM_ID, THRM_CTRL_READ, THRM_TRPCLR_REG, &regval);
